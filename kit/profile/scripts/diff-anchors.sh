...
#
# Part of the delivery rails (see workflows/RAILS.md). This is the line-anchor
# scaffolding shared by the grader and correctness-review rails: it turns a PR diff
# into a canonical, machine-readable list of changed-line ranges, one per contiguous
# added/modified hunk:
#
#   src/Foo.cs:120-145
#   src/Foo.cs:201-201
...
# non-reproducible findings that drift line numbers and wander outside the change.
# Feeding it THIS list as the canonical anchor set forces every finding to pin to a
# real changed line and keeps review scoped to the diff. Same input in, same anchors
# out — the technique is borrowed from deterministic code-review scaffolding (e.g.
# alibaba/open-code-review); only the technique, not the tooling.
#
# Output is the anchor list on stdout (sorted, de-duplicated); a one-line count
# summary goes to stderr so stdout stays pure and pipeable. Deleted files are excluded
# (a finding cannot anchor to a line that no longer exists); pure deletions inside a
# surviving file are likewise not emitted — review them via the diff itself, anchored
# to the nearest surviving line.
#
# This script is stack-agnostic: it operates on git hunks, not on any language. The
# only thing to adapt per repo is the BASE_REF default and the pathspec you pass
# (e.g. your source root) — see Usage.
#
...
BASE_REF="${BASE_REF:-main}"   # <<DEFAULT_BRANCH>>
PATHSPECS=()
...
# landed on the base branch since the branch point. Try the ref as given, then the
# origin/-prefixed form (CI checks out with a remote-tracking base).
resolve_base() {
  local ref="$1"
