...
#
# Part of the delivery rails (see workflows/RAILS.md). This is "branch protection as
# code": the source of truth is the ruleset JSON, and this script is the only
# sanctioned way to push it live. It is idempotent — it creates the ruleset if
# absent, updates it in place (by name) if present.
#
# It NEVER runs automatically. A human runs it deliberately, after reading the diff
# between the desired JSON and what is live. Applying live branch protection is a
# HIGH-risk, outward-facing action; treat it as such.
#
...
# Find an existing ruleset with the same name (rulesets are not unique by name in the
# API, but we manage exactly one with this name). Fail LOUDLY if the lookup itself
# errors — a swallowed auth/rate failure would look like "no ruleset" and make us POST
# a duplicate instead of PUT-updating the existing one.
