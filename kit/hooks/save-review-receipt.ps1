...
#
# Maturity: STABLE. No external tooling beyond git.
#
# Env knobs:
#   RAILS_REVIEW_KINDS — comma-separated set of allowed review kinds. Default:
#                        code-review,simplify. Must match the kinds the review gate checks.
#
...
# Validate $Kind against the configured set (keeps this in lockstep with the review gate).
$kindsRaw = $env:RAILS_REVIEW_KINDS
if (-not $kindsRaw) { $kindsRaw = 'code-review,simplify' }
$allowed = $kindsRaw -split '[,\s]+' | Where-Object { $_ }
if ($allowed -notcontains $Kind) {
  [Console]::Error.WriteLine("save-review-receipt: '$Kind' is not an allowed review kind ($($allowed -join ', ')).")
  exit 1
}
