...
_Every term the Delivery Standard leans on, defined in plain words. Written for anyone joining a
pod, onboarding from the client side, or reading the standard for the first time and hitting a
word they don't recognize._
...
**How to read this.** Read tier 1 once. Tiers 2 and 3 are lookup.

[Must-know](#must-know--the-terms-that-carry-the-method) ·
[Roles and rituals](#roles-and-rituals) ·
[Implementation names](#implementation-names--files-jobs-and-tools-in-todays-kit)

---

## Must-know — the terms that carry the method

_Sixteen terms. If you know these, you can follow every other page in the standard; everything
below builds on them._
...
**The author is never the sole approver** (also **the non-author check**). The rule that whoever
produced a change doesn't get to be the only one who signs it off — not for code, not for a harness
change, not for a spec, not for this document. A fresh grader and a non-author human carry the
verdict. This is the practical answer to "how do you trust code you didn't write." Survives every
team size (see [the team](team.md)).
...
**Receipts.** The committed record a gate or ritual leaves behind — a review receipt, a sign-off
line, a decision-log row. The standard's rule is that machine gates produce artifacts and human
decisions produce receipts: if a person decided something and nothing in the repo says so, the
decision didn't happen (see [the build loop](build-loop.md) and `docs/phase-9-monitoring.md`).
...
the trap (standard, section 9). The **review-wait tripwire** is the enforcement side: the agreed
threshold that, once crossed, stops new work starting until the review queue clears (see
...
**Decision list.** The running list of product decisions nobody has made yet ("you haven't decided
X"), each needing a named human answer on the client clock agreed in Phase 1. Every item records
the question, the date raised, the named client owner, the clock date, and the answer once given.
It lives in the repo with the phase artifacts, is worked at intent triage, and goes to every
steering (see [the build loop](build-loop.md)).
...
**Intent triage.** The weekly 60-minute refinement session the Pod Lead runs: stories become ready
specs — vague lines sharpened, silent decisions surfaced onto the decision list, risk tiers
assigned, oversized stories split, the backlog ordered. The place Definition of Ready is enforced,
and the only door a story has into the loop (standard, section 5.1).

**Orchestrator.** The role that directs an agent through Delegate: sets the bounds (scope, context,
one agent or many), corrects and approves the plan in plan mode before any code, and runs no more
than two streams at once. Not a separate headcount — Orchestrators and Checkers swap per change,
and nobody orchestrates and checks the same change. See [the team](team.md).
...
**Reopening a decision.** Revising a signed decision from inside the loop, with a record: a named
owner, the reason, the two-business-day clock, and the impact listed. A design revision goes
through as a HIGH-risk spec and the old ADR is marked superseded rather than edited, so the history
of what was believed when survives. What is not allowed is the quiet drift — code that no longer
matches a record nobody updated (see [when things change](when-things-change.md)).
...
**Spike.** Bounded, throwaway work run to answer a question the pod cannot yet answer — what you
run when a story can't be made ready because nobody knows enough yet. Opened by the Pod Lead at
intent triage against a named unknown, with a time or token box written down. The code lives on a
`spike/` branch and cannot merge (a CI check enforces it); the written finding is the deliverable
(see [the build loop](build-loop.md), section 3a).

**Sponsor.** The client executive who owns the outcome and the one success metric. Decides the
things only the client can: PO mode (a named PO or proxy mode), the tooling and data boundary,
go/no-go to production, and the close. Sees the biweekly steering — demo, scorecard, decision list
— and never activity metrics. Their obligations are written into the SOW (standard, section 12).
...
## Implementation names — files, jobs and tools in today's kit

_These are the names things have in the kit we ship today; the method does not depend on any of
them. Borrowed industry terms sit here too._
...
to stop. A **blocking** hook refuses to let the agent finish while tests fail or the build is
broken; it isn't optional or persuadable. The **Stop hook** (`stop-gate`) is that blocking hook in
today's kit: it refuses to let an agent finish a turn with red tests or a broken build, and is the
single highest-value automation the kit ships (standard, section 5.2).
...
**The rails.** The agentic CI/CD and DevOps pipeline every change rides. The rails are the five
PR-time gates — **ci**, **grader**, **correctness**, **security**, **deploy-dev** — plus the branch
protection that makes them mandatory; with them come the merge bar, deploy and promotion, and the
agent-safe IaC funnel. Three further workflow files exist in the repo (`deploy-promote`,
`dependency-scan`, `rails-telemetry`) but none of them fire on a PR, so they are not gates a
change must clear. Not a numbered phase; a standing standard. Governing principle: **agent
proposes, gate disposes.** Deep-dive: [the-rails.md](the-rails.md).
