...
| **The rhythm**           | Numbered phases open and close the engagement; in between runs this continuous build loop. Phases end with gates (automated checks plus a named human sign-off); the loop has **no batch artifact gate** — it has a merge bar that every change clears, and it is left when a human declares the backlog feature-complete. |
